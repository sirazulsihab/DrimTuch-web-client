...
export default function Payouts() {
  ...
  const approvePayout = async (id) => {
    ...
    try {
      const res = await fetch(`/api/payouts/approve?id=${id}`, { method: "POST" });
      const data = await res.json();
  ...
  const rejectPayout = async (id) => {
    ...
    try {
      const res = await fetch(`/api/payouts/reject?id=${id}`, { method: "POST" });
      const data = await res.json();
    ...
    <div>
      ...
      <table className="w-full border">
        ...
        <tbody>
          {payouts.map((p) => (
            <tr key={p._id}>
              <td className="border px-2 py-1">{p.affiliateName}</td>
