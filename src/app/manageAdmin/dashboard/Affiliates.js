...
export default function Affiliates() {
  ...
  const handleApprove = async (id) => {
    try {
      const token = localStorage.getItem("adminToken");
      // const res = await fetch(`http://localhost:5000/api/admin/affiliates/approve/${id}`, {
      const res = await fetch(`https://drimtuch-server.onrender.com/api/admin/affiliates/approve/${id}`, {
        method: "POST",
        headers: {
